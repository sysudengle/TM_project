...
int InitSettings()
{
    ...
    x = (MOLDYN_DOUBLE *) malloc(numMoles * NDIM * sizeof(MOLDYN_DOUBLE));
    f = (MOLDYN_DOUBLE *) malloc(numMoles * NDIM * sizeof(MOLDYN_DOUBLE));
    vh = (MOLDYN_DOUBLE *) malloc(numMoles * NDIM * sizeof(MOLDYN_DOUBLE));

    ver_t* tag;
    for (i = 0; i < numMoles * NDIM; i++) {
    ...
    inter = (MOLDYN_INT *) malloc(MAXINTERACT * 2 * sizeof(MOLDYN_INT));

    for (i = 0; i < MAXINTERACT * 2; i++) {
    	inter[i].__meta = init_tag();
    }

    epot.__meta = init_tag();
    vir.__meta = init_tag();
    count.__meta = init_tag();
    vel.__meta = init_tag();
    ekin.__meta = init_tag();
    ninter.__meta = init_tag();
...
void InitCoordinates(int numMoles, int siz, double perturb) {
  ...
  for ( n =0; n< npoints; n++) {
    ...
    x[IND(0,n + npoints)] = x_base + offset;
    x[IND(1,n + npoints)] = y_base + offset;
    x[IND(2,n + npoints)] = z_base;

    x[IND(0,n + npoints * 2)] = x_base + offset;
    x[IND(1,n + npoints * 2)] = y_base;
    x[IND(2,n + npoints * 2)] = z_base + offset;

    x[IND(0,n + npoints * 3)] = x_base;
    x[IND(1,n + npoints * 3)] = y_base + offset;
    x[IND(2,n + npoints * 3)] = z_base + offset;
  }
}
...
int PrintCoordinates(INPARAMS int numMoles) {
  ...
  for (i = 0; i < numMoles; i++) {
     printf("%d: %f:%f -- %f:%f -- %f:%f\n", i, (double)x[IND(0,i)], (double)vh[IND(0,i)], (double)x[IND(1,i)], (double)vh[IND(1,i)], (double)x[IND(2,i)], (double)vh[IND(2,i)]);
  }
