...
int InitSettings()
{
    ...
    inter = new MOLDYN_INT[MAXINTERACT * 2];
...
int PrintCoordinates(INPARAMS int numMoles) {
  int i = 0;
  for (i = 0; i < numMoles; i++) {
     printf("%d: %f:%f -- %f:%f -- %f:%f\n", i, (double)molecules[i].x, (double)vh[IND(0,i)], (double)molecules[i].y, (double)vh[IND(1,i)], (double)molecules[i].z, (double)vh[IND(2,i)]);
  }
