...
void dlfree(Void_t* m)
{
  unsigned int i;

  // get back to the real start
  unsigned char *p = ((unsigned char*)m) - ZONE_SIZE - sizeof(size_t);
...
Void_t* public_rEALLOc(Void_t* m, size_t bytes) {
  if (MALLOC_PREACTION != 0) {
    ...
    if (!m) {
      ...
      size_t prevSize = *((size_t*)(((char*)m) - ZONE_SIZE - sizeof(size_t))) - (2*ZONE_SIZE + sizeof(size_t));
    ...
    if (inuse(p)) {
      ...
      // total size
        ...
        enum HeapWalkOpts opts =
          func(((char*)chunk2mem(p)) + sizeof(size_t) + ZONE_SIZE,
